...
picam2.set_controls({"FrameRate": 60})  # 프레임 속도 설정
# 카메라 시작
picam2.start()
...
while True:
    ...
    if key == ord('r'):
        ...
        # 동영상 저장을 위한 VideoWriter 객체 설정
        out = cv2.VideoWriter('recorded_video.mp4', cv2.VideoWriter_fourcc(*'MP4V'), 20.0, (640, 480))
        print("녹화 시작")

    # 녹화 중이면 프레임을 동영상에 추가
    if recording:
        out.write(img)  # 현재 프레임을 동영상에 추가

        # 지정된 녹화 시간(초)이 지나면 녹화 종료
        if time.time() - record_start_time > record_duration:
            recording = False
            out.release()
            print(f"녹화 종료 - recorded_video.mp4 저장 완료")
